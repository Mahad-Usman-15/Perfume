...
export default function Product({params}:{params:{slug:string}}){
     const perfume=perfumes.find((perf)=>perf.slug===params.slug)
   const [image,setisimage]=useState<StaticImageData>(perfume?.image||perfumes[0].image)
...
<div className="flex flex-col">
    <div className="mt-20 grid grid-cols-1 md:grid-cols-2 items-center justify-between px-2 py-2 gap-4">
  ...
  <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible w-full lg:w-auto">
    {perfume.images.map((img, index) => (
      <Image
        ...
        src={img}
        alt="Thumbnail"
        className="w-24 h-24 min-w-[6rem] object-cover cursor-pointer hover:opacity-80 transition-opacity rounded"
      />
    ))}
  </div>
...
<div className="flex flex-col items-start gap-3 sm:gap-4 md:gap-5 lg:gap-7 px-1 py-1">
    <h1 className="font-normal text-2xl md:text-3xl text-gray-600">{perfume.Name}</h1>
     <span className="flex items-center gap-1">
          {[1, 2, 3, 4, 5].map((index) => (
            <FaStar key={index} className="fill-yellow-500 text-base" />
          ))}
        </span>
         <span className="flex flex-col md:flex-row items-center gap-3 sm:gap-4 md:gap-5 lg:gap-7 ">
              <h1 className="text-2xl md:text-3xl font-normal text-red line-through">{perfume.cancelledprice}</h1>
            <h1 className="text-2xl  md:text-3xl font-normal text-red-600">${perfume.price}</h1>
          
        </span>
        <span className="grid grid-cols-2 md:grid-cols-3  grid-rows-2  gap-3">
            {
                ["400,000+ Happy customers","Easy Return & Exchange","Fast delivery all over Pakistan","Product ready to ship"].map((item,index)=>(
                    <span key={index} className="px-1 py-1 flex items-center justify-center border border-black rounded-full">
<label className="text-center text-lg text-black">{item}</label>
                    </span>
                ))
...
{/* <span className="flex flex-col md:flex-row items-center gap-3 md:gap-6">
    <label className="text-2xl font-normal text-black">Quantity</label>
    <span className="flex flex-row items-center gap-2">
<button onClick={decrement} className="bg-white hover:bg-gray-300 px-2 py-2">-</button>
<label className="text-black">{quantity}</label>
<button onClick={increment} className="bg-white hover:bg-gray-300 px-2 py-2">+</button>
    </span>
</span> */}
       
 <button 
            data-item-id={perfume.id}
            data-item-name={perfume.Name}
            data-item-price={perfume.price}
            data-item-image={perfume.image}
            data-item-url="/" 
            className="snipcart-add-item w-full text-white transition bg-black hover:bg-gray-800 px-2 py-2 m-1">
Add to Cart
        </button>
